...
static int should_append_binding(jl_module_t *m, jl_binding_t *b, int non_public, int imported, int usings_explicit, int generated, int deprecated)
{
    ...
    if (isgenerated && !generated)
        return 0;
    if (b->imported == BINDING_KIND_CONST_IMPORT) // CONST_IMPORT is bit of a hybrid that's gated by the `imported` flag.
        return imported;
    if (b->imported == BINDING_KIND_GLOBAL || jl_binding_is_some_constant(b)) {
    ...
    } else if (b->imported == BINDING_KIND_IMPORTED) {
        return imported;
    } else if (b->imported == BINDING_KIND_EXPLICIT) {
